...
globalkeys = my_table.join(
    ...
    -- Tag browsing
    awful.key({ altkey, "Ctrl" }, "Left",   awful.tag.viewprev       ),
    awful.key({ altkey, "Ctrl" }, "Right",  awful.tag.viewnext       ),
    awful.key({ altkey, "Ctrl" }, "x", awful.tag.history.restore),
    ...
    awful.key({ altkey, "Control" }, "o", function () lain.util.useless_gaps_resize(1) end,
    ...
    awful.key({ modkey, "Shift"   }, "a", function () awful.layout.set(awful.layout.suit.tile)                end,
    ...
    awful.key({ altkey, "Ctrl" }, "c", function () awful.spawn("/usr/bin/diodon") end,
              {description = "run clipboard manager", group = "launcher"}),
...
clientkeys = my_table.join(
    ...
    awful.key({ modkey   }, "s",  	  toggle_roll_up_or_shade),
    awful.key({ modkey }, "g", leaved.keys.shiftOrder),
    awful.key({ modkey, "Shift" }, "y", leaved.keys.splitH), --split next horizontal
...
awful.rules.rules = {
    ...
    -- Floating clients rules.
    { rule_any = {
        instance = { --run xprop - instance is 1st string of WM_CLASS, 2nd string is class
          "maya.bin",
          },
    ...
    -- Titlebars
    { rule_any = { type = { "dialog", "normal", "utility" } },
